...
class SignInBloc extends BaseBloc<SignInData, Command> {
  ...
  void _dynamicLink() => FirebaseDynamicLinks.instance.getInitialLink().then((link) {
        if (link == null) return;
        final login = Uri.parse(link.link.queryParameters['continueUrl']!).queryParameters['login'] ?? '';
