...
class ChatBloc extends BaseBloc {
  ...
  late final Stream<ChatData> stream;
  ...
  Stream<ChatData> _getStream(String login, String name) async* {
    ...
    WebSocket.connect('ws://$ip:4442/ws').then((s) => _onSocket(s, login, name, props)).onError((error, stackTrace) {
      ...
      _ctr.add(Command.MAINTENANCE);
    });
    var chatData = const ChatData();
    await for (final state in _ctr.stream)
      switch (state) {
        case Command.DIALOG:
    ...
    switch (msg[TYPE]) {
      case REPORT:
        ...
        if (reports.length == 15)
          ...
          FirebaseFirestore.instance.doc('$USER/$login/$REPORT/$_peerId').set({NAME: _peerName});
        }
        break;
      case PEER:
        final peerId = msg[ID];
  ...
  Future<void> _block(login, Set<String> reports) async {
    ...
    FirebaseFirestore.instance.collection('$USER/$login/$REPORT').get().then((col) {
      col.docs.forEach(_del);
    });
    var doc = await FirebaseFirestore.instance.doc('$BLOCKED_USER/$login').get();
